...
describe("StakingRewards", () => {
  ...
  it("notifyRewardAmount: full", async () => {
    ...
    expect(rewardAmount).to.be.eq(
      reward.div(REWARDS_DURATION).mul(REWARDS_DURATION)
    );
  });

  it("stakeWithPermit", async () => {
    // creating a temp wallet as the easiest way to have access to a private key for this.
    const temporaryStakerWallet = ethers.Wallet.createRandom().connect(
      ethers.provider
    );
    ...
    await accounts[1].sendTransaction({
      to: temporaryStakerWallet.address,
      value: ethers.utils.parseEther("5000"),
    });

    // get permit
    const nonce = await (stakingToken as Contract).nonces(
      temporaryStakerWallet.address
    );
    ...
    const digest = await getApprovalDigest(
      stakingToken,
      {
        owner: temporaryStakerWallet.address,
    ...
    const { v, r, s } = ecsign(
      ...
      Buffer.from(temporaryStakerWallet.privateKey.slice(2), "hex")
    );

    await stakingRewards
      .connect(temporaryStakerWallet)
      .stakeWithPermit(stake, deadline, v, r, s);
    ...
    const rewardAmount = await rewardsToken.balanceOf(
      temporaryStakerWallet.address
    );
    expect(reward.sub(rewardAmount).lte(reward.div(10000))).to.be.true; // ensure result is within .01%
    expect(rewardAmount).to.be.eq(
  ...
  it("notifyRewardAmount: ~half [ @skip-on-coverage ]", async () => {
    ...
    await mineBlock(
      ethers.provider,
      startTime.add(endTime.sub(startTime).div(2)).toNumber()
    );
    ...
    const rewardAmount = await rewardsToken.balanceOf(staker.address);
    expect(reward.div(2).sub(rewardAmount).lte(reward.div(2).div(10000))).to.be
      .true; // ensure result is within .01%
    expect(rewardAmount).to.be.eq(
      reward.div(REWARDS_DURATION).mul(endTime.sub(stakeStartTime))
    );
  ...
  it("notifyRewardAmount: two stakers", async () => {
    ...
    await stakingToken
      ...
      .approve(stakingRewards.address, stake);
    ...
    const rewardAmount = await rewardsToken.balanceOf(staker.address);
    const secondRewardAmount = await rewardsToken.balanceOf(
    ...
    expect(
      totalReward
        .div(4)
        .sub(secondRewardAmount)
        .lte(totalReward.div(4).div(10000))
    );
  });
});
